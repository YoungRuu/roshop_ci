...
class Backend_Controller extends MX_Controller {
    public $auth;
	  public function __construct(){
        ...
        if($this->session->has_userdata('user_login')){
          $this->auth = $this->session->userdata('user_login');
        }
    }
  ...
  protected $_config_pagi = array(
    ...
    public function get_config_pagi($name_model , $condition = '' , $string_search =''){
      $limit = 10;
      $current_page = isset($_GET['page']) ? $_GET['page'] : 1;
      $total_record = $this->$name_model->countAll($condition);
      ...
      $range_page   = 5;
      if($string_search != ''){
        $link_full  = base_url(uri_string()).'?page={page}&'.$string_search;
        $link_first = base_url(uri_string()).'?'.$string_search;
      }else{
        $link_full  = base_url(uri_string()).'?page={page}';
        $link_first = base_url(uri_string());
      }
      /*---Pagination---*/
      $pagination = array(
